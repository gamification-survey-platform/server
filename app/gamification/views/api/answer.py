...
from app.gamification.serializers.answer import (
    AnswerSerializer,
    ArtifactFeedbackSerializer,
...
class ArtifactAnswerList(generics.ListCreateAPIView):
    ...
    def get(self, request, artifact_review_pk, *args, **kwargs):
        answer = Answer.objects.filter(artifact_review_id=artifact_review_pk).order_by("pk")
        ...
        if question.question_type == Question.QuestionType.SLIDEREVIEW:
            ...
            for question_option in question_options:
                if Answer.objects.filter(question_option=question_option, artifact_review=artifact_review).count() > 0:
                    answer = Answer.objects.filter(question_option=question_option, artifact_review=artifact_review)
                    answers.extend(answer)
            serializer = self.get_serializer(answers, many=True)
            return Response(serializer.data)
        ...
        if (
            ...
            for question_option in question_options:
                if len(answer_texts) == 0:
                    break
                if Answer.objects.filter(question_option=question_option, artifact_review=artifact_review).count() > 0:
                    answer = Answer.objects.get(question_option=question_option, artifact_review=artifact_review)
                    # answer {answer_text= 'b', question_option= question_option(optionChoice=b, question=question)
                    # artifact_review= aritfact_review}
            ...
            while len(answer_texts) != 0:
                ...
                answer = Answer(
                    answer_text=current_answer, question_option=question_option, artifact_review=artifact_review
                )
                answer.save()
            serializer = self.get_serializer(answer)
            return Response(serializer.data)
            ...
            answers = Answer.objects.filter(question_option=question_option, artifact_review=artifact_review)
...
class ArtifactResult(generics.ListAPIView):
    ...
    def get(self, request, artifact_pk, *args, **kwargs):
        ...
        for artifact_review in artifact_reviews:
            answers = Answer.objects.filter(artifact_review=artifact_review)
            for answer in answers:
                if (
                    answer.question_option.question.text == "Your confidence"
                    and answer.question_option.question.question_type == Question.QuestionType.NUMBER
                ):
        ...
        for section in sections:
            ...
            for question in section.questions:
                ...
                if (
                    ...
                    or question.question_type == Question.QuestionType.SCALEMULTIPLECHOICE
                ):
                    question_options = question.options
                    for question_option in question_options:
                        answer_text = question_option.option_choice.text
                        count = Answer.objects.filter(
                            artifact_review__in=artifact_reviews, question_option=question_option
                        ).count()
                        if count > 0:
                            answers[section.title][question.text]["answers"].append({answer_text: count})
                    ...
                    for artifact_review in artifact_reviews:
                        ...
                        if text_answers.count() > 0:
                            count += 1
                            res += int(text_answers[0].answer_text) * int(confidence[artifact_review.pk])
                    if confidence["sum"] != 0:
                    ...
                    answers[section.title][question.text]["answers"] = defaultdict(list)
                    question_option = get_object_or_404(QuestionOption, question=question)
                    for artifact_review in artifact_reviews:
                        ...
                        for answer in text_answers:
                            answers[section.title][question.text]["answers"][answer.page].append(answer.answer_text)
                    ...
                    for artifact_review in artifact_reviews:
                        ...
                        for answer in text_answers:
                            answers[section.title][question.text]["answers"].append(answer.answer_text)
    ...
    def get(self, request, artifact_pk, *args, **kwargs):
        ...
        for answer in answers:
            if answer.question_option.question.question_type == "MULTIPLECHOICE":
                if (
                    answer.question_option.question.text
                    not in result["sections"][answer.question_option.question.section.title].keys()
                ):
                    result["sections"][answer.question_option.question.section.title][
                        answer.question_option.question.text
                ...
                if (
                    answer.question_option.question.text
                    not in result["sections_scale"][answer.question_option.question.section.title].keys()
                ):
